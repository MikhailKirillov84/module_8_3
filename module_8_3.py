...
class Car:
    def __init__(self, model, __vin, __numbers):
        ...
        self.model = model
        # создадим условие, где будем проверять атрибут "__vin" методом "isinstance" принадлежность к числовому(int) типу,
        ...
        # создадим условие, где будем проверять количество передаваемых символов в атрибут "__vin" в диапазоне,
        # если условие не выполняется, будем с помощью ключевого слова "raise" передавать исключение с сообщением.
        if not (9999999 >= __vin and __vin >= 1000000):
            raise IncorrectVinNumber('Неверный диапазон для vin номера')
        # если же условия выполняются, создадим атрибут объекта "__vin" - vin номер автомобиля (целое число).
        else:
            self.__vin = __vin
        # создадим условие, где будем проверять атрибут "__numbers" методом "isinstance" принадлежность к
        ...
        # создадим условие, где будем проверять длину передаваемых значений в атрибут "__numbers" функцией "len",
        # если условие не выполняется, будем с помощью ключевого слова "raise" передавать исключение с сообщением.
        if len(__numbers) != 6:
            raise IncorrectCarNumbers('Неверная длина номера')
        # если же условия выполняются, создадим атрибут объекта "__numbers" - номер автомобиля (строка).
        else:
...
class IncorrectVinNumber(Exception):
    ...
    # создадим метод "__is_valid_vin", который принимает один атрибут "__vin", передадим атрибуту значение передаваемое
    # при вызове метода.
...
class IncorrectCarNumbers(Exception):
    ...
    def __init__(self, message):
        # создадим атрибут объекта "message" - сообщение.
        self.message = message
    # создадим метод "__is_valid_numbers", который принимает один атрибут "__numbers", передадим атрибуту значение
